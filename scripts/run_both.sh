#!/bin/bash
# 孔明本地双策略比较（从本地 output/ 目录读取已有的凤雏/刘秀数据）
# 本机执行
cd "$(dirname "$0")/.."
DATE=$(date "+%Y-%m-%d")
...
# 检查刘秀数据
if [ -f "output/liuxiu/reports/market_snapshot_${DATE}.md" ]; then
    echo "刘秀数据: 存在" >> "logs/kongming_${DATE}.log"
else
    echo "刘秀数据: 未找到（尝试从10.26.0.5拉取）" >> "logs/kongming_${DATE}.log"
    scp "chenhe@10.26.0.5:~/股票市场扫描规则/output/liuxiu/reports/market_snapshot_${DATE}.md" \
        "output/liuxiu/reports/" 2>/dev/null && \
        echo "刘秀数据已从10.26.0.5拉取" >> "logs/kongming_${DATE}.log"
fi
